...
new GridBuilder( _grid , {
   ...
   fields: [
      ...
      },
      {
         ...
         renderer: function( row ) {
            if (row.setting_variable == 'school_name') {
               return _SCHOOL_LEVEL >= 5 ? 'Nama Perguruan Tinggi' : 'Nama Instansi';
            }
            if (row.setting_variable == 'school_status') {
               return _SCHOOL_LEVEL >= 5 ? 'Status Perguruan Tinggi' : 'Status Instansi';
            }
            if (row.setting_variable == 'headmaster') {
               return _HEADMASTER;
            }
...
new FormBuilder( _form1 , {
   controller:'settings/school_profile',
   fields: [
      { label:'NPIP', name:'setting_value', placeholder:'Nomor Pokok Instansi Pemerintah' }
   ]
});
...
new FormBuilder( _form2 , {
   controller:'settings/school_profile',
   fields: [
      { label:'Nama ' + (_SCHOOL_LEVEL >= 5 ? 'Perguruan Tinggi' : 'Instansi'), name:'setting_value' }
   ]
});
...
new FormBuilder( _form4 , {
   controller:'settings/school_profile',
   fields: [
      { label:'Status Instansi', name:'setting_value', type:'select', datasource:DS.SchoolStatus }
   ]
});
